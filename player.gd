...
#@onready var flashlight: SpotLight3D = $XROrigin3D/RightController/Flashlight
...
func _physics_process(delta: float) -> void:
	...
	# no gravity for now (flat ground)
	velocity.y = 0.0
	move_and_slide()
...
func _get_move_input() -> Vector2:
	...
	# On OpenXR, pushing stick up usually gives negative Y -> flip so up = forward
	input_vec.y = input_vec.y

	return input_vec


func _move_player(input_vec: Vector2) -> void:
	# Head-relative forward on XZ plane
...
func _start_level0() -> void:
	...
	_set_bubble_cursor_enabled(false)   # ❗ no bubble in Level 0
	if flashlight:
...
func _start_level1_animals() -> void:
	...
	_set_dark_jungle(false)             # brighter area
	_set_bubble_cursor_enabled(true)    # ✅ bubble cursor ON here
	if flashlight:
		flashlight.visible = false

	total_animals = get_tree().get_nodes_in_group("animal").size()
	animals_fed = 0
	print("LEVEL 1: Feed all animals")


func _start_level1_equip() -> void:
	phase = GamePhase.LEVEL1_EQUIP
	_set_dark_jungle(true)
	_set_bubble_cursor_enabled(false)   # bubble OFF again
	if flashlight:
		flashlight.visible = true        # when you hook it up
	equipment_count = 0
	print("Dark jungle again: find 5 tools")


func _start_level2_river() -> void:
	phase = GamePhase.LEVEL2_RIVER
	_set_dark_jungle(false)
	_set_bubble_cursor_enabled(false)   # no bubble in level 2
	if flashlight:
		flashlight.visible = false
...
func _set_bubble_cursor_enabled(enabled: bool) -> void:
	var big_radius  := 0.4   # Level 1 bubble size
	var small_radius := 0.12 # normal hand size

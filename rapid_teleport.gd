...
#
	#var head_pos := global_transform.origin
	#var dist := head_pos.distance_to(hit_point)
	#if dist > max_teleport_distance:
		#teleport_marker.visible = false
		#return
#
	#teleport_marker.global_position = hit_point
...
#func _start_teleport() -> void:
	#if not teleport_marker.visible:
		#print("[RapidTP] No valid teleport target")
		#return
#
	#_target_pos = teleport_marker.global_position
	#_is_teleporting = true
	#teleport_marker.visible = false
	#print("[RapidTP] Teleport start -> ", _target_pos)
#
...
#
	#var dist := dir.length()
		...
		#global_position = Vector3(_target_pos.x, global_position.y, _target_pos.z)
		#_is_teleporting = false
